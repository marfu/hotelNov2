...
public class ChambreFactureBean implements Serializable {
    ...
    public void loadDetailsFacture() {
        if (chambreOccupationSelected.size() == 0) {

            FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Erreur!", "Veuillez selectionner une ligne de facture !"));
            ...
            System.out.println("Deposit:xxxxxxxxxxxxxxx" + deposit);
            ...
            System.out.println("Client id:xxxxxxxxxxxxxxx" + clientByChoose.get(0).getCliId());
            deposit = tDepositeService.findTDepositeByUser(clientByChoose.get(0).getCliId());

            //montantTTC = montantTotal + (montantTotal * 18 / 100);
            montantTTC = montantTotal - deposit;

            RequestContext context = RequestContext.getCurrentInstance();
            context.execute("PF('multiCarDialog').show();");
        }
    ...
    public void findDepository() {
        System.out.println("Montant deposit xxxxxxxxxxxxxxxxxxxxxxxx" + deposit);
    ...
    public void chooseRemise() {
        ...
        System.out.println("xxxxxxxxxxxxxxxxxxxxxxxx" + tauxRemise);
        montantRemise = (montantTotal * tauxRemise) / 100;
        montantTTC = montantTotal - (montantTotal * tauxRemise / 1000) - deposit;
