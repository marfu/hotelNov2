...
//System.out.println("xxxxxxxxxx");
        for (TCategorieChambre temp : listCategorieChambre) {
            ...
            InfoChambreDto dtoOOcc = tChambreService.findChambreOccupee(temp.getCatChambreId(), dateDebut, dateFin);
            InfoChambreDto dtoReserv = tChambreService.findChambreReservee(temp.getCatChambreId(), dateDebut, dateFin);
            List<TChambre> tchambreTotal = tChambreService.listChambreByCat(temp.getCatChambreId());
            List<TChambre> tchambre = tChambreService.listChambreByEtatAndCat(EtatChambreEnum.HORS_SERVICE, temp.getCatChambreId());

            // InfoChambreDto dto = tChambreService.findDetailsChambre(temp.getCatChambreId(), dateDebut, dateFin);
            dtoUtil.setHorsService(tchambre.size());

            dtoUtil.setTotalChambreOccupe(dtoOOcc.getTotalChambreOccupe());
            dtoUtil.setTotalChambreReserve(dtoReserv.getTotalChambreOccupe());
            dtoUtil.setTypeChambre(temp.getCatChambreLib());

            dtoUtil.setTotalChambre(tchambreTotal.size());
            dtoUtil.setTotalChambreLibre(dtoUtil.getTotalChambre() - (dtoUtil.getTotalChambreOccupe() + dtoUtil.getTotalChambreReserve() + dtoUtil.getHorsService()));
